...
import { Vector2D, Vector3D } from "./types.js";
const canvas = document.getElementById("c");
const context = canvas.getContext("2d");
if (!context) {
    throw new Error("2D context not supported");
}
const point = new Vector3D(1, 1, 2);
function generateAsteroid(radius, roughness) {
...
function generateAsteroidField(count) {
    const asteroids = [];
    for (let i = 0; i < count; i++) {
        ...
        asteroids.push({
            vertices,
            edges,
            position: new Vector3D((Math.random() - 0.5) * 250, (Math.random() - 0.5) * 250, (Math.random() - 0.5) * 250),
            rotation: new Vector3D(0, 0, 0),
            rotationSpeed: new Vector3D((Math.random() - 0.5) * 0.01, (Math.random() - 0.5) * 0.01, (Math.random() - 0.5) * 0.01),
