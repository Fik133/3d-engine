...
export function getProjectionMatrix(): ProjectionMatrix {
...
function logic() {
  projectionMatrix.matrix = engineMath.buildProjectionMatrix(
    projectionMatrix.aspect,
    projectionMatrix.fov,
  );
  ...
  for (let x = -gridSize; x < gridSize; x++) {
    for (let z = -gridSize; z < gridSize; z++) {
      ...
      edges.forEach((edge) => {
        ...
        const worldSpace1 = modelMatrix.multiply4DVector(
          new Vector4D(v1.x * scale, v1.y * scale, v1.z * scale, 1),
        );
        const worldSpace2 = modelMatrix.multiply4DVector(
          new Vector4D(v2.x * scale, v2.y * scale, v2.z * scale, 1),
        );


        const cameraSpace1 = engineMath.project(
          worldSpace1.x,
          worldSpace1.y,
          worldSpace1.z,
        );
        const cameraSpace2 = engineMath.project(
          worldSpace2.x,
          worldSpace2.y,
          worldSpace2.z,
        );
